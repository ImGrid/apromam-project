...
// Utilidades para consultas PostGIS especificas del sistema
// Consultas geoespaciales comunes para coordenadas de Bolivia
export class PostGISQuery {
  // Valida coordenadas dentro de los limites de Bolivia
  // Usa configuracion geografica del pais
  static async validateCoordinates(
    latitude: number,
    longitude: number
  ): Promise<{ valid: boolean; within_bolivia: boolean }> {
    const query = {
      text: `
        SELECT 
          ST_IsValid(ST_Point($1, $2, 4326)) as valid,
          ST_Within(
            ST_Point($1, $2, 4326),
      ...
      values: [
        longitude,
        latitude, // PostGIS: X=longitud, Y=latitud
        -69.651047,
        -22.896133,
        -57.453621,
        -9.680567, // Limites Bolivia
      ],
      name: "validar_coordenadas",
    };

    const result = await ReadQuery.findOne<{
      valid: boolean;
      within_bolivia: boolean;
    }>(query);

    return result || { valid: false, within_bolivia: false };
  }

  // Calcula distancia entre dos puntos GPS en metros
  // Usa algoritmo ST_Distance_Sphere para precision
  static async calculateDistance(
    ...
    const query = {
      ...
      `,
      values: [lng1, lat1, lng2, lat2],
      name: "calcular_distancia",
    };

    const result = await ReadQuery.findOne<{ distance_meters: number }>(query);
    return result?.distance_meters || 0;
  }

  // Encuentra parcelas cerca de un punto especifico
  // Optimizado para busquedas de proximidad GPS
  static async findNearbyPoints(
    centerLat: number,
    centerLng: number,
    radiusMeters: number,
    tableName: string = "parcelas"
  ): Promise<any[]> {
    const query = {
      text: `
        SELECT *,
               ST_Distance_Sphere(coordenadas, ST_Point($2, $1, 4326)) as distance_meters
        FROM ${tableName}
        WHERE ST_DWithin(
          coordenadas::geography,
          ST_Point($2, $1, 4326)::geography,
          $3
        )
        ORDER BY distance_meters
      `,
      values: [centerLat, centerLng, radiusMeters],
      name: `buscar_cercanos_${tableName}`,
    };

    return ReadQuery.execute(query);
  }
}
